...
import { getAllBooks, getBooksForGenre } from "../../api/booksApi";
import ReactModal from "../ReactModal";
import AddBookForm from "../forms/AddBookForm";
import BookRow from "./BookRow";
import { useParams } from "react-router-dom";

const GenreBooksTable = () => {
  ...
  const fetchBooks = useCallback(() => {
    getBooksForGenre({ setBooks, setApiStatus, id, page });
  }, [page]);
    ...
    <div className="flex flex-col py-2 px-1">
      <div className="flex items-center justify-between">
        <div className="flex gap-3 items-center">
          <h1 className="text-2xl my-3 font-bold">Genre Books</h1>
        </div>
        <button
          onClick={() => {
      ...
      {allBooks && (
        <div className="flex items-center gap-3 my-3">
          ...
          <button
            ...
            disabled={allBooks?.books?.length === 0 || apiStatus === "loading"}
            className="border p-2 bg-slate-900 rounded-lg disabled:opacity-70 text-white h-8 w-8 flex items-center justify-center"
          >
            +
          </button>
...
export default GenreBooksTable;
